...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    # For large a, sigmoid(a) rounds to exactly 1.0 in floating point,
    # so strict increase is only checkable away from saturation.
...
def test_other(x: float) -> None:
    ...
    # relu is the same as taking the max with zero
    assert relu(x) == max(x, 0.0)
